...
(function() {
  var ngNav, ngNavBreadcrumbsDirective, ngNavLinkBuilderDirective, ngNavMenuDirective, ngNavProvider;
  ...
  ngNav = {
    ...
    /**
     * @method setRoutes
     * @param {Object} routes See {{#crossLink "ngNav/_routes:attribute"}}{{/crossLink}} 
     * @chainable
     */
    ...
    /**
     * @method setMenu
     * @param {Object} menu See {{#crossLink "ngNav/_menu:attribute"}}{{/crossLink}} 
     * @chainable
     */
     ...
     * @return {Array} {{#crossLink "ngNav/_breadcrumbs:attribute"}}breadcrumbs array{{/crossLink}}
     */
    ...
    getLink: function(routeName, params) {
      ...
      if (route) {
        ...
        return console.log('can not create link for non existent route');
      }
    }
  };
  ...
  ngNavProvider = {
    ...
    $get: [
      '$rootScope', '$route', '$routeParams', function($rootScope, $route, $routeParams) {
        ngNav.bindBreadcrumbsGenerator($rootScope, $route, $routeParams);
        ngNav.bindMenuGenerator($rootScope, $route, $routeParams);
        return ngNav;
      }
    ]
  };
  ...
  ngNavBreadcrumbsDirective = function(ngNav) {
    var directive;
    directive = {
      restrict: 'E',
      transclude: true,
      ...
      link: function(scope, el, attrs, ctrl, transcludeFn) {
        ...
        return scope.$watch(function() {
          return ngNav.getBreadcrumbs();
        }, function() {
          return scope.breadcrumbs = ngNav.getBreadcrumbs();
        });
      }
    };
    return directive;
  ...
  ngNavMenuDirective = function(ngNav) {
    var directive;
    directive = {
      restrict: 'AE',
      transclude: true,
      ...
      link: function(scope, el, attrs, ctrl, transcludeFn) {
        ...
        return scope.$watch(function() {
          return ngNav.getMenu(scope.menuName);
        }, function() {
          return scope.menu = ngNav.getMenu(scope.menuName);
        });
      }
    };
    return directive;
  ...
  ngNavLinkBuilderDirective = function(ngNav) {
    var directive;
    directive = {
      restrict: 'A',
      scope: {
        routeName: '@ngNavLinkBuilder',
        params: '=ngNavLinkParams'
      },
      link: function(scope, el, attr) {
        var updateHref;
        updateHref = function() {
          return attr.$set('href', ngNav.getLink(scope.routeName, scope.params));
        };
  ...
  angular.module('ngNav', ['ngRoute']).provider('ngNav', ngNavProvider).directive('ngNavBreadcrumbs', ['ngNav', ngNavBreadcrumbsDirective]).directive('ngNavMenu', ['ngNav', ngNavMenuDirective]).directive('ngNavLinkBuilder', ['ngNav', ngNavLinkBuilderDirective]);
